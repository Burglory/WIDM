...
final class Layer3Decoder {
	...
	public Layer3Decoder(Bitstream stream, Header header, SynthesisFilter filter1, SynthesisFilter filter2,
		...
		sfreq = header.sampleFrequency() + ((header.version() == Header.VERSION_MPEG1) ? 3
	...
	private void fastInvMdct(double[] in, double[] out, int blockType) {
		...
		if (blockType == 2) {
			...
			tmp0ob = in[2 + 1] * 1.9696155060244f + i7s + in[10 + 1] * 1.2855752193731f
					+ in[14 + 1] * 0.68404028665134f;
			tmp1ob = (in[2 + 1] - in[10 + 1] - in[14 + 1]) * 1.732050808f;
			tmp2ob = in[2 + 1] * 1.2855752193731f - i7s - in[10 + 1] * 0.68404028665134f
					+ in[14 + 1] * 1.9696155060244f;
			tmp3ob = in[2 + 1] * 0.68404028665134f - i7s + in[10 + 1] * 1.9696155060244f
